...
const DeliNode = memo(({ id, data }: any) => {
  const mini = data.miniCollapsed;
  const rules: DeliRule[] = data.rules || [];
  ...
  const updateRule = (index: number, field: keyof DeliRule, val: any) => {
  ...
  return (
    <BaseNode
      ...
      title="Шеф Deli"
      bgColor="#f43f5e" // Рожевий для Deli
      type="deliNode"
      width="w-[320px]"
    >
      <Handle
        ...
        id="in"
        style={getHandleStyle('#f43f5e', mini ? '50%' : '20px', mini)}
        className="!left-[-6px]"
      />
      ...
      {!mini && (
        <div className="p-3 space-y-3">
          ...
          <div className="bg-muted/30 p-2 rounded-lg border border-border/50 space-y-2">
            <div className="text-[10px] font-bold text-rose-400 uppercase mb-2">Список страв ({Object.keys(DELI_RECIPES_DATA).length})</div>
            
            <div className="space-y-1.5 max-h-[300px] overflow-y-auto pr-1 custom-scrollbar">
              {rules.map((rule, i) => {
                const recipeBase = DELI_RECIPES_DATA[rule.recipeName];
                if (!recipeBase) return null;
                
                const isExpanded = expandedRecipes[rule.recipeName];
                
                return (
                  <div key={rule.recipeName} className={`rounded border transition-colors ${rule.enabled ? 'bg-slate-800/80 border-rose-500/50' : 'bg-slate-900/50 border-slate-800'}`}>
                    ...
                    <div className="flex items-center justify-between p-2">
                      ...
                      >
                        ...
                        <span className={`text-[11px] font-medium ${rule.enabled ? 'text-rose-100' : 'text-slate-500'}`}>
                          {rule.recipeName}
                        </span>
                      </div>
                              ...
                              return (
                                <div key={ing} className="flex items-center justify-between gap-2">
                                  ...
                                  <div className="flex items-center gap-1.5">
                                    ...
                                    />
                                    <span className="text-[9px] font-mono text-rose-300 w-8 text-right">={required}</span>
                                  </div>
                                </div>
                              );
                            })}
