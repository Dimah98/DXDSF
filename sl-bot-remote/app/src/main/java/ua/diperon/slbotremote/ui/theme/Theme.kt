package ua.diperon.slbotremote.ui.theme

import android.os.Build
import androidx.compose.foundation.isSystemInDarkTheme
import androidx.compose.material3.MaterialTheme
import androidx.compose.material3.darkColorScheme
import androidx.compose.material3.dynamicDarkColorScheme
import androidx.compose.material3.dynamicLightColorScheme
import androidx.compose.material3.lightColorScheme
import androidx.compose.runtime.Composable
import androidx.compose.ui.platform.LocalContext
import androidx.compose.ui.graphics.Color

private val DarkColorScheme =
  darkColorScheme(
    primary = GlassPrimary,
    secondary = GlassSecondary,
    tertiary = GlassAccent,
    background = GlassBg,
    surface = GlassBg,
    onPrimary = Color.White,
    onSecondary = Color.White,
    onTertiary = Color.White,
    onBackground = GlassOnSurface,
    onSurface = GlassOnSurface,
    surfaceVariant = GlassTerminal,
    onSurfaceVariant = GlassOnSurfaceVariant,
    error = GlassError,
    onError = Color.White,
    outline = GlassBorder,
    outlineVariant = GlassBorderSubtle,
    surfaceContainer = GlassCard,
    surfaceContainerHigh = GlassCardHover,
    surfaceContainerHighest = GlassCardActive,
    inverseSurface = GlassOnSurface,
    inverseOnSurface = GlassBg,
    inversePrimary = GlassPrimaryLight
  )

private val LightColorScheme =
  lightColorScheme(
    primary = GlassPrimary,
    secondary = GlassSecondary,
    tertiary = GlassAccent,
    background = GlassBg,
    surface = GlassBg,
    onPrimary = Color.White,
    onSecondary = Color.White,
    onTertiary = Color.White,
    onBackground = GlassOnSurface,
    onSurface = GlassOnSurface,
    surfaceVariant = GlassTerminal,
    onSurfaceVariant = GlassOnSurfaceVariant,
    error = GlassError,
    onError = Color.White,
    outline = GlassBorder,
    outlineVariant = GlassBorderSubtle,
    surfaceContainer = GlassCard,
    surfaceContainerHigh = GlassCardHover,
    surfaceContainerHighest = GlassCardActive,
    inverseSurface = GlassOnSurface,
    inverseOnSurface = GlassBg,
    inversePrimary = GlassPrimaryLight
  )

@Composable
fun MyApplicationTheme(
  darkTheme: Boolean = isSystemInDarkTheme(),
  // Dynamic color is available on Android 12+
  dynamicColor: Boolean = true,
  content: @Composable () -> Unit,
) {
  val colorScheme =
    when {
      dynamicColor && Build.VERSION.SDK_INT >= Build.VERSION_CODES.S -> {
        val context = LocalContext.current
        if (darkTheme) dynamicDarkColorScheme(context) else dynamicLightColorScheme(context)
      }

      darkTheme -> DarkColorScheme
      else -> LightColorScheme
    }

  MaterialTheme(colorScheme = colorScheme, typography = Typography, content = content)
}
